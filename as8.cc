...
void readProtocol(unsigned int ts, int protocol, int d, map<unsigned int, map<int, float> > &protocolMap){
...
void readSize(unsigned int ts, float bps, int d, map<unsigned int, map<float, float> >  &flowMap,map<unsigned int, double> &traffic_at_Ti){
...
void updateQ(map<unsigned int, float> &phi, map<unsigned int, double> &traffic_at_Ti)
{
...
void readIP(unsigned int ts, unsigned int ip, int d, map<unsigned int, map<unsigned int, float>> &dstIPMap){
...
void readPort(unsigned int ts, int port,int d, map<unsigned int, map<int, float> > &dstPortMap){
...
void KthLargest(unsigned int ts, string keyy, float score,map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> &flowScore){
...
void
amonProcessingNfdump (char* line,map<unsigned int, map<float, float> > &flowMap, map<unsigned int, map<int, float> > &protocolMap,map<unsigned int, map<unsigned int, float>> &dstIPMap, map<unsigned int, map<int, float> > &dstPortMap, map<unsigned int, double> &traffic_at_Ti)
{
    ...
    readSize(firstSeen, bps,d,flowMap,traffic_at_Ti);
    readProtocol(firstSeen, protocol,d,protocolMap);
    readIP(firstSeen, dstIP,d, dstIPMap);
    readPort(firstSeen, dstPort,d, dstPortMap);
    free(delimiters);
}

void scoreProcessing (char* line,map<unsigned int, map<float, float> > &flowMap, map<unsigned int, map<int, float> > &protocolMap,map<unsigned int, map<unsigned int, float>> &dstIPMap, map<unsigned int, map<int, float> > &dstPortMap,map<unsigned int, float> &totalMagnitude,map<unsigned int, int> &countFlow,map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> &flowScore){
  ...
  /* 2|1453485557|768|1453485557|768|6|0|0|0|2379511808|44694|0|0|0|2792759296|995|0|0|0|0|2|0|1|40 */
    ...
    for (int i = 0; i < d; i++){
        ...
        KthLargest(ts, keyy, score,flowScore);
...
void computeRemaining(map<unsigned int, float> &totalMagnitude, map<unsigned int, int> &countFlow, map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> &flowScore, map<unsigned int, vector<pair<float, string>>> &flowCDF,map<unsigned int, float> &remaining){
    map<unsigned int, float>::iterator i;
...
void computeCDF(map<unsigned int, float> &totalMagnitude, map<unsigned int, int> &countFlow, map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> &flowScore, map<unsigned int, vector<pair<float, string>>> &flowCDF,map<unsigned int, float> &remaining){
    map<unsigned int, float>::iterator i;
...
void printAlert(map<unsigned int, float> &totalMagnitude, map<unsigned int, int> &countFlow, map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> &flowScore, map<unsigned int, vector<pair<float, string>>> &flowCDF,map<unsigned int, float> &remaining,map<unsigned int, float> &phi){
    map<unsigned int, float>::iterator i;
...
cout<<endl<<flowMap.size();
cout<<endl<<protocolMap.size();
cout<<endl<<dstIPMap.size();
cout<<endl<<dstPortMap.size();
cout<<endl<<traffic_at_Ti.size();
cout<<endl<<phi.size();
cout<<endl<<totalMagnitude.size();
cout<<endl<<countFlow.size();
cout<<endl<<flowScore.size();
cout<<endl<<flowCDF.size();
cout<<endl<<remaining.size();
cout<<endl<<q_map.size();
cout<<endl<<flowProfile.size();
cout<<endl<<protocolProfile.size();
cout<<endl<<dstIPProfile.size();

}

*/
void clearMemory(map<unsigned int, map<float, float> > &flowMap, map<unsigned int, map<int, float> > &protocolMap,map<unsigned int, map<unsigned int, float>> &dstIPMap, map<unsigned int, map<int, float> > &dstPortMap,map<unsigned int, float> &totalMagnitude,map<unsigned int, int> &countFlow,map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> &flowScore, map<unsigned int, vector<pair<float, string>>> &flowCDF, map<unsigned int, float> &remaining,map<unsigned int, double> &traffic_at_Ti, map<unsigned int, float> &phi){


   map<unsigned int, map<float, float> >::iterator i;
   map<float, float>::iterator j;
   map<int, float>::iterator k;
   map<unsigned int, float>::iterator l;

   for(i = flowMap.begin() ; i != flowMap.end(); i++){
           j = flowMap[i->first].begin();
           while(j != flowMap[i->first].end()){
                  j = flowMap[i->first].erase(j);
           }
           k = protocolMap[i->first].begin();
           while(k != protocolMap[i->first].end()){
		  k = protocolMap[i->first].erase(k);
           }
           l = dstIPMap[i->first].begin();
           while(l != dstIPMap[i->first].end()){
                  l = dstIPMap[i->first].erase(l);
           }
           k = dstPortMap[i->first].begin();
           while(k != dstPortMap[i->first].end()){
                 k = dstPortMap[i->first].erase(k);
           }

           while( !flowScore[i->first].empty() )
           {
               flowScore[i->first].pop();
           }
           //flowScore[i->first].clear();
           flowCDF[i->first].clear();
    }
...
map<unsigned int, map<float, float> > flowMap;               
map<unsigned int, map<int, float> > protocolMap;           
map<unsigned int, map<unsigned int, float>> dstIPMap;      
map<unsigned int, map<int, float> > dstPortMap;            
map<unsigned int, double> traffic_at_Ti;
map<unsigned int, float> phi;
map<unsigned int, float> totalMagnitude;
map<unsigned int, int> countFlow;
map<unsigned int, priority_queue<flow, vector<flow>, greater<flow>>> flowScore;
map<unsigned int, vector<pair<float, string>>> flowCDF;
map<unsigned int, float> remaining;
           ...
           { 
            ...
            if (ctr % 2 == 1){      // (odd) do the reading (pm, CLP)
                ...
                while (fgets(line, MAXLINE, nf) != NULL)
                {
                    ...
                    amonProcessingNfdump(line,flowMap,protocolMap,dstIPMap,dstPortMap,traffic_at_Ti);
                ...
                while (fgets(line, MAXLINE, nf) != NULL)
                {
                    ...
                    continue;
                        ...
                        scoreProcessing(line,flowMap,protocolMap,dstIPMap,dstPortMap,totalMagnitude,countFlow,flowScore);

                }
                cout<<endl<<"Finished Scoring";
                computeRemaining(totalMagnitude,countFlow,flowScore,flowCDF,remaining);
                cout<<endl<<"Finished Compute Remaining";
                computeCDF(totalMagnitude,countFlow,flowScore,flowCDF,remaining);
                cout<<endl<<"Finished CDF";
                updateQ(phi,traffic_at_Ti);
                cout<<endl<<"Finished UpdateQ";
                printAlert(totalMagnitude,countFlow,flowScore,flowCDF,remaining,phi);    
                cout<<endl<<"Done Printing Alerts";
                clearMemory(flowMap,protocolMap,dstIPMap,dstPortMap,totalMagnitude,countFlow,flowScore,flowCDF,remaining,traffic_at_Ti,phi);
